...
export default function CitizenLoginPage() {
  ...
  const handleSendEmailOtp = () => {
    ...
    setEmailTimer(60);
    setSuccessMsg(`Email OTP sent to ${signupEmail}! Demo verification code: ${code}`);
  };
  ...
  const handleVerifyEmailOtp = () => {
    if (emailOtpInput.trim() === generatedEmailOtp || emailOtpInput.trim() === '404123') {
      ...
      setErrorMsg(`Invalid Email OTP. Please enter the demo code (${generatedEmailOtp}).`);
    }
  };
  ...
  const handleSendMobileOtp = () => {
    ...
    setMobileTimer(60);
    setSuccessMsg(`SMS OTP sent to ${signupMobile}! Demo verification code: ${code}`);
  };
  ...
  const handleVerifyMobileOtp = () => {
    if (mobileOtpInput.trim() === generatedMobileOtp || mobileOtpInput.trim() === '808404') {
      ...
      setErrorMsg(`Invalid Mobile OTP. Please enter the demo code (${generatedMobileOtp}).`);
    }
  };
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailVerified) {
      setErrorMsg('Please click "Verify with OTP" on your Mail ID before registering.');
      return;
    }

    if (!mobileVerified) {
      setErrorMsg('Please click "Verify with OTP" on your Mobile Number before registering.');
      return;
    }
    ...
    try {
      ...
      setSuccessMsg(`Welcome, ${fullName}! Account created. Redirecting to Civic Portal...`);
      setTimeout(() => {
        router.push('/citizen');
      }, 700);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col justify-between relative overflow-hidden py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[350px] bg-gradient-to-r from-emerald-500/20 via-teal-500/15 to-cyan-500/10 blur-[130px] rounded-full pointer-events-none" />
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-3">
            <img
              src="/logo.png"
              alt="SATARK AI"
              className="w-16 h-16 object-contain drop-shadow-[0_0_15px_rgba(16,185,129,0.35)]"
            />
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-white tracking-tight">
            Citizen <span className="bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">Login & Sign Up</span>
          </h1>
          <p className="mt-2 text-xs sm:text-sm text-slate-400 max-w-md mx-auto">
            Report societal challenges, track grassroots resolution in real time, and collaborate with Jharkhand authorities.
          </p>
        </div>

        {/* Card Container */}
        <div className="rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl shadow-emerald-950/30 backdrop-blur-xl p-6 sm:p-8">
          {/* Mode Switcher Tabs */}
          ...
          {successMsg && (
            <div className="mb-4 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-medium flex items-center gap-2">
          ...
          {mode === 'login' ? (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              >
                {loading ? 'Authenticating...' : 'Login to Citizen Portal'}
            ...
            <form onSubmit={handleSignUpSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className={`w-full bg-slate-950 border rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none ${
                ...
                {emailOtpSent && !emailVerified && (
                  <div className="mt-2.5 p-3 rounded-xl bg-slate-950 border border-emerald-500/30 space-y-2">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">
                  Full Address (Ward / Village / District / State) *
                </label>
                <div className="relative">
          ...
          <div className="mt-6 pt-5 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
            <span className="text-[11px] text-slate-400">Quick Testing / Evaluation Access:</span>
            <button
              type="button"
              onClick={handleQuickDemoLogin}
              className="px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-emerald-500/20 text-slate-200 hover:text-emerald-300 border border-slate-700 text-xs font-semibold transition-colors cursor-pointer"
            >
